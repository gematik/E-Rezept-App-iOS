...
extension AppConfiguration {
    /// Holds all environment variables set via the `.env` files at compile time.
...
extension AppConfiguration.Environment {
    ...
    static let ORGAN_DONATION_REGISTER_PU_URL_TEMP: String =
...
public struct AppConfiguration: Equatable, Sendable {
    ...
    public let idp: URL
    public let idpAdditionalHeader: [String: String]
    public let idpDefaultScopes: [String]

    public let trustAnchor: TrustAnchor
    // erp fd
    ...
    public let erp: URL
    public let erpAdditionalHeader: [String: String]
    ...
    public let fhirVzdAdditionalHeader: [String: String]

    // eRezept backend
    public let eRezept: URL
    ...
    public struct Server: Sendable {
        ...
        public init?(url: String, header: [String: String]) {
...
// [REQ:gemSpec_Krypt:A_24469] Gematik Root CA 3 as a trust anchor has to be set in the program code
...
/// The default app configuration is the production one
public let defaultConfiguration: AppConfiguration = environmentTU ?? environmentPU
#elseif DEFAULT_ENVIRONMENT_RU
/// The default app configuration is the production one
public let defaultConfiguration: AppConfiguration = environmentRU ?? environmentPU
#elseif DEFAULT_ENVIRONMENT_RU_DEV
/// The default app configuration is the production one
public let defaultConfiguration: AppConfiguration = environmentRUDEV ?? environmentPU
#else
...
#else
/// The default app configuration is the production one
public let defaultConfiguration: AppConfiguration = environmentPU
#endif

#endif

#if TEST_ENVIRONMENT
/// Mapping of all available configurations
public let configurations: [String: AppConfiguration] = [
    "TU": environmentTU,
    "RU": environmentRU,
