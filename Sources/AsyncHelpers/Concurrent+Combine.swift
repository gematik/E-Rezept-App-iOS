...
public enum AsyncError: Error, Equatable {
    public static func ==(lhs: AsyncError, rhs: AsyncError) -> Bool {
        switch (lhs, rhs) {
...
extension Publisher {
    ...
    public func asyncResult<E2: Swift.Error>(_ transformError: CaseKeyPath<E2, Self.Failure>) async throws
    ...
    public func asyncResult() async -> Result<Self.Output, AsyncError> {
        await _asyncResult()
    }
...
extension Future where Failure == Error {
    /// Bridges from  structured concurrent code to `Publisher`.
    public static func createWithEscapedDependencies(operation: @escaping () async throws -> Output) -> Self {
...
extension Future where Failure == Never {
    /// Bridges from  structured concurrent code to `Publisher`.
    public static func createWithEscapedDependencies(operation: @escaping () async -> Output) -> Self {
        withEscapedDependencies { continuation in
            Self { promise in
                Task {
