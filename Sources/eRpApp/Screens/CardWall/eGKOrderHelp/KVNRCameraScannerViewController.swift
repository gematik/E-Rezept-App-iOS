...
class KVNRCameraScannerViewController: UIViewController {
    var kvnrScanned: ((KVNR) -> Void)?
    ...
    func recognizeTextHandler(request: VNRequest, error _: Error?) {
        ...
        for visionResult in results {
            ...
            if let result = candidate.string.extractKVNR() {
                ...
                if let box = try? candidate.boundingBox(for: range)?.boundingBox {
                    ...
                    numberIsSubstring = !(range.lowerBound == candidate.string.startIndex && range
                        .upperBound == candidate.string.endIndex)
                }
            }
        ...
        if let sureNumber = numberTracker.getStableString() {
            DispatchQueue.main.async { [weak self] in
                self?.kvnrScanned?(sureNumber as KVNR)
    ...
    func addBox(in rect: CGRect, color: CGColor) {
        ...
        DispatchQueue.main.async {
            ...
            for boxGroup in boxGroups {
                ...
                for box in boxGroup.boxes {
                    ...
                    self.addBox(in: rect, color: color)
                }
            }
        }
    }
...
extension KVNRCameraScannerViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
...
extension String {
    func extractKVNR() -> (Range<String.Index>, String)? {
        if (self as KVNR).isValid,
...
extension KVNRCameraScannerViewController {
    class StringTracker {
