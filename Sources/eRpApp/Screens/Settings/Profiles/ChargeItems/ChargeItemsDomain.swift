...
struct ChargeItemsDomain: ReducerProtocol {
    ...
    struct Destinations: ReducerProtocol {
        enum State: Equatable {
            ...
            case alert(ErpAlertState<ChargeItemsDomain.Action>)
    ...
    enum Action: Equatable {
        ...
        enum Response: Equatable {
            case fetchChargeItems(ChargeItemDomainServiceFetchResult)
            case authenticate(ChargeItemDomainServiceAuthenticateResult)
            case grantConsent(ChargeItemsDomainServiceGrantResult)
            case revokeConsent(ChargeItemsDomainServiceRevokeResult)
        }
    ...
    @Dependency(\.chargeItemsDomainService) var chargeItemsService: ChargeItemsDomainService
    ...
    func core(into state: inout State, action: Action) -> EffectTask<Action> {
        ...
        case let .select(chargeItem):
            state.destination = .chargeItem(.init(chargeItem: chargeItem.original))
            return .none

        case .fetchChargeItems:
            return chargeItemsService.fetchChargeItems(for: state.profileId)
                .first()
                .eraseToEffect()
                .map(Action.Response.fetchChargeItems)
                .map(Action.response)
                .receive(on: schedulers.main)
                .eraseToEffect()
                .cancellable(id: Token.fetchChargeItems)
        case let .response(.fetchChargeItems(result)):
            switch result {
