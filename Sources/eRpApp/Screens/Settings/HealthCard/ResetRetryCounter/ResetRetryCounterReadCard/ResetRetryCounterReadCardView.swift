...
struct ResetRetryCounterReadCardView: View {
    let store: ResetRetryCounterReadCardDomain.Store

    init(store: ResetRetryCounterReadCardDomain.Store) {
        self.store = store
    }
    ...
    var body: some View {
        WithViewStore(store) { viewStore in
            ...
            .alert(
                store.scope(
                    state: (\ResetRetryCounterReadCardDomain.State.route)
                        .appending(path: /ResetRetryCounterReadCardDomain.Route.alert)
                        .extract(from:)
                ),
                dismiss: .nothing
            )
...
struct ResetRetryCounterReadCardView_Previews: PreviewProvider {
    static var previews: some View {
        NavigationView {
            ResetRetryCounterReadCardView(
                store: ResetRetryCounterReadCardDomain.Dummies.store
            )
        }
    }
}
