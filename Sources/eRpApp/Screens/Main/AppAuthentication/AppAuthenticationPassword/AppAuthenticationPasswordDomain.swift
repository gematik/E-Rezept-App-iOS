...
enum AppAuthenticationPasswordDomain {
    ...
    enum Action: Equatable {
        ...
        case closeAfterPasswordVerified
    }
    ...
    static let domainReducer = Reducer { state, action, environment in
        ...
        case .loginButtonTapped:
            guard let success = try? environment.appSecurityPasswordManager.matches(password: state.password),
                  success == true else {
                state.lastMatchResultSuccessful = false
                return .none
            }
            return Effect(value: .closeAfterPasswordVerified)
