...
extension AnyReducer where Action == AppDomain.Action, State == AppDomain.State, Environment == AppDomain.Environment {
    ...
    private func recordActionsAfterStateChange() -> Self {
        .init { state, action, environment in
            ...
            case .main(action: .prescriptionList(action: .loadLocalPrescriptionsReceived)),
                 .main(action: .prescriptionList(action: .loadRemotePrescriptionsAndSaveReceived)):
                if !state.main.prescriptionListState.prescriptions.isEmpty {
