...
// sourcery: CodedError = "590"
public enum LocalStorageBundleParsingError: Swift.Error {
...
extension ModelsR4.Bundle {
    ...
    func parseErxChargeItemIDs() throws -> [String] {
        ...
        try entry?.compactMap {
            ...
            guard let identifier = chargeItem.id?.value?.string else {
                throw LocalStorageBundleParsingError.parseError("Could not parse id from charge item.")
            }
            return identifier
        } ?? []
    }
    ...
    func parseErxChargeItem( // swiftlint:disable:this function_body_length cyclomatic_complexity
        ...
        guard let entry = entry?.first, // for now we assume that there is only one charge item
            ...
            throw LocalStorageBundleParsingError.parseError("Could not parse the charge item.")
        }
        ...
        guard let pharmacyId = pharmacy?.davOrganizationIdentifier
            ...
            throw LocalStorageBundleParsingError.parseError("Could not parse organization identifier.")
        }

        guard let pharmacyName = pharmacy?.name?.value?.string
        else {
            throw LocalStorageBundleParsingError.parseError("Could not parse organization name.")
        }

        guard let pharmacyAddress = pharmacy?.twoLineAddress
        else {
            throw LocalStorageBundleParsingError.parseError("Could not parse organization address.")
        }

        guard let pharmacyCountry = pharmacy?.address?.first?.country?.value?.description
        else {
            throw LocalStorageBundleParsingError.parseError("Could not parse organization address country.")
        }
        ...
        guard let totalAdditionalFee = invoice?.totalAdditionalFee
            ...
            throw LocalStorageBundleParsingError.parseError("Could not parse invoice totalGross additionalFee.")
        }

        guard let totalGross = invoice?.totalGross?.value?.value?.decimal
        else {
            throw LocalStorageBundleParsingError.parseError("Could not parse invoice totalGross value.")
        }

        guard let currency = invoice?.totalGross?.currency?.value?.string
        else {
            throw LocalStorageBundleParsingError.parseError("Could not parse invoice totalGross currency.")
        }
...
extension ModelsR4.Bundle {
    ...
    func parseSupportingInformationBundle(
        ...
        guard let reference = chargeItem.supportingInformation?
            ...
            throw LocalStorageBundleParsingError.parseError("Could not parse bundle reference with key: \(bundleKey).")
        }

        guard let bundle = findResource(with: reference, type: ModelsR4.Bundle.self)
        else {
            throw LocalStorageBundleParsingError.parseError("Could not parse bundle with key: \(bundleKey).")
        }
    ...
    func productionSteps(special _: Bool = false) throws -> [DavInvoice.Production.Ingredient] {
        try lineItem?.map {
            ...
            guard let price = $0.priceComponent?.first?.amount?.value?.value?.decimal
                ...
                throw LocalStorageBundleParsingError.parseError("Could not parse invoice priceComponent amount value.")
            }
    ...
    func chargeableItems(separation: Bool = false) throws -> [DavInvoice.ChargeableItem] {
        try lineItem?.map {
            guard let factor = $0.priceComponent?.first?.factor?.value?.decimal
                ...
                throw LocalStorageBundleParsingError.parseError("Could not parse invoice priceComponent factor.")
            }
