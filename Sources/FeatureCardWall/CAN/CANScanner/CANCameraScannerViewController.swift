...
class CANCameraScannerViewController: UIViewController {
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        // Handle device orientation in the preview layer (iOS 17+ only).
    ...
    func setupCamera() {
        guard let captureDevice = AVCaptureDevice.default(
            ...
            Logger.cardWall.debug("Could not create capture device.")
            return
        }
        ...
        guard let deviceInput = try? AVCaptureDeviceInput(device: captureDevice) else {
            Logger.cardWall.debug("Could not create device input.")
            return
        }
        ...
        if captureSession.canAddOutput(videoDataOutput) {
            ...
            Logger.cardWall.debug("Could not add VDO output")
            return
        }

        // Dont set zoom and but autofocus to help focus on very small text.
        do {
            ...
            Logger.cardWall.debug("Could not set zoom level due to error: \(error)")
            return
        }
    ...
    func captureOutput(_: AVCaptureOutput,
        ...
        if let pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
            ...
            do {
                ...
                Logger.cardWall.debug("\(error.localizedDescription)")
            }
        }
    }
}
