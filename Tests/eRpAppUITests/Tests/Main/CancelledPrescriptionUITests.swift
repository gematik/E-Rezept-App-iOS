...
@MainActor
final class CancelledPrescriptionUITests: XCTestCase, Sendable {
    ...
    override func setUp() async throws {
        ...
        app.launchEnvironment["UITEST.SCENARIO_NAME"] = "CancelledPrescriptionUITests"
    ...
    func testCancelledPrescription_cannotRedeemButDelete() {
        let name = "HomöopaTea"
        let subtitle = "Gelöscht am: 09.07.2024"

        XCTAssertTrue(app.staticTexts[subtitle].exists)

        let tabBar = TabBarScreen(app: app)
        _ = tabBar.tapPrescriptionsTab()
            .tapDetailsForPrescriptionNamed(name)

        XCTAssertTrue(app.buttons[subtitle].exists)
    }
}
