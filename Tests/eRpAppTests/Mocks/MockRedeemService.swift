...
class MockRedeemService: RedeemService {
    init() {}
    ...
    func redeem(_ orders: [Order]) -> AnyPublisher<IdentifiedArrayOf<OrderResponse>, RedeemServiceError> {
        redeemCallsCount += 1
        redeemParameters = orders
        return redeemOrdersClosure.map { $0(orders) } ?? redeemReturnValue
    }
}
